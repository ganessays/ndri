...
// import this file if json _encode() is not supported in the server
...
class Filemanager {
	...
	function Filemanager($config){
	 ...
	 function error($string,$textarea=false) {
		...
		if($textarea) {
			echo '<textarea>' .$jsonEncoder->encode($array). '</textarea>';
		} else {
			echo $jsonEncoder->encode($array);
		}
		die();
	}
	 ...
	 function add() {
		...
		echo '<textarea>' . $jsonEncoder->encode($response) . '</textarea>';
		die();
	}
